...
final class SSS_MLB_Prepared_Inputs_Repository extends SSS_MLB_Base_Repository {
    ...
    public function get_latest_game_environment_input_for_event(int $event_id): ?array {
        ...
        $sql = $this->wpdb->prepare(
            "SELECT pi.*, fs.feature_set_key
             ...
             WHERE pi.event_id = %d
               AND pi.subject_type = 'event'
               AND fs.feature_set_key = 'phase1_game_environment'
             ORDER BY pi.prepared_at_utc DESC, pi.id DESC
             LIMIT 1",
            $event_id
        );
    ...
    public function upsert_game_environment_input(array $payload): int {
    ...
    public function upsert_validation_game_input(array $payload): int {
        return $this->upsert_game_environment_input($payload);
    }
}
